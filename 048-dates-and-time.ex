...
@doc """
    Четвертый модуль DateTime не имеет такого недостатка, как NaiveDateTime, то есть DateTime поддерживает часовые пояса, но за этим нужно следить. 
    Многие функции из этого модуля требуют базу данных часовых поясов. По умолчанию они используют базу данных, которую возвращает функция 
    Calendar.get_time_zone_database, а эта база поддерживает только пояс “Etc/UTC”. Если нужно менять часовые пояса, советую присмотреться к 
    библиотеке tzdata. Теперь изучим DateTime:
"""
...
@doc """
    Как было видно из примеров, время и даты можно задавать через строковые метки (сигили), для Time - ~T, для Date - ~D, для NaiveDateTime - ~N, для 
    DateTime - ~U соответственно:
...
###### ASSIGNMENT ######
@doc """
    Создайте функцию shift_days, которая принимает структуры Time, DateTime, NaiveDateTime, Date и смещает ее на количество переданных дней. 
    Дни могут быть отрицательным числом:
...
defmodule Solution do
  ...
  # NaiveDateTime → NaiveDateTime.add/3 (в секундах)
  def shift_days(%NaiveDateTime{} = dt, days) do
    NaiveDateTime.add(dt, days * 86_400)
  end

  # DateTime → DateTime.add/3 (в секундах)
  def shift_days(%DateTime{} = dt, days) do
    DateTime.add(dt, days * 86_400)
  end

  # Time → ничего не меняем (нет даты)
